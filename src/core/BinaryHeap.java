...
public class BinaryHeap<E extends Comparable<E>> {
	...
	public boolean present(E el) {
		if (this.array.contains(el)) {
			return true ; 
		}
		else {
			return false ; 
		...
		while (true) {
			...
			for (int i = 0; i < nb_remove; i++) {
				...
				if (removed1 != removed2) {
					System.out.println("Ouch : expected " + removed1
							+ "  .. but got " + removed2);
					System.exit(1);
				}
			}
			...
			if (heap.size() != queue.size()) {
				System.out.println("Ouch : heap size = " + heap.size()
			...
			if (count > 1000000) {
				System.out.println("" + count
						+ " items successfully compared. Heap size : "
						+ heap.size());
				count = 0;
			}
		}
